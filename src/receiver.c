	...
	if(readed == -1){
		...
		if(!pkt){
			...
			if(pkt_get_type(pkt) != PTYPE_DATA){
				fprintf(stderr, "Was nos a DATA packet\n");
				pkt_del(pkt);
				return -1;
			}
			fprintf(stderr, "Packed received with sqnum %d\n", pkt_get_seqnum(pkt));
			...
			if(pkt_get_tr(pkt) == 1){
				...
				if(not_in_sequence == -10){
					...
					if(list->head != NULL){
						fprintf(stderr, "Sending ack %d. The first seqnum of the list is %d\n", waited_seqnum, pkt_get_seqnum(list->head->packet));
					}
					send_ack(ack, sfd, last_timestamp);
					pkt_del(ack);
				}
...
/*
 * Free all the memory of the packets which were in the window but
 * out of sequence, if the receiver times out
 */
void free_timeout(list_t* list){
	pkt_t* packet = pop_element_queue(list);
	while(packet != NULL){
		pkt_del(packet);
		packet = pop_element_queue(list);
	}
}
